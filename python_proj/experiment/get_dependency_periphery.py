...
dependencies_path = f"./data/libraries/{eco}-libraries-1.6.0-2020-01-12/predictors/dependencies.csv"
dependencies_file = open(dependencies_path, "w+")
dependencies_writer = writer(dependencies_file)
dependencies_writer.writerow(['Focal Project', 'Other Project'])
...
for entry in dependencies_reader:
    ...
    if other_project_name in included_projects:
        projects_that_depend_on_core.add(other_id)
        added_dependency = True
...
with open(rpr.input_path, "r") as input_file:
    ...
    for entry in input_reader:
        ...
        entry_id = entry[project_id_index]
        repo_name = entry[repo_name_index]

        added_dependency = False

        if entry_id in projects_that_core_depends_on:
            focal_to_other_file.write(f'{repo_name}\n')
            added_dependency = True

        if entry_id in projects_that_depend_on_core:
            other_to_focal_file.write(f'{repo_name}\n')
            added_dependency = True

        # Adds id to repo name if dependency is relevant.
        if added_dependency:
            id_to_repo_name[entry_id] = repo_name
