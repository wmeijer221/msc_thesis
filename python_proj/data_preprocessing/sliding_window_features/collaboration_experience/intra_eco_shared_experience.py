...
from python_proj.utils.exp_utils import get_repository_name_from_source_path, SOURCE_PATH_KEY, get_integrator_key
from python_proj.data_preprocessing.sliding_window_features.collaboration_experience.shared_experience import SharedExperienceFeature
...
class IntraProjectSharedExperienceFeature(SharedExperienceFeature):
    ...
    """
    def __init__(self, nested_source_keys: list[str | Callable[[dict], str]], nested_target_keys: list[str | Callable[[dict], str]], is_inversed: bool = False) -> None:
        super().__init__(nested_source_keys, nested_target_keys, is_inversed)
        self._shared_experience = SafeDict(default_value=SafeDict, 
    ...
    def _get_nodes(self, entry: dict, nested_key: list[str | Callable[[dict], str]]) -> list[int]:
...
if __name__ == "__main__":
    from python_proj.data_preprocessing.sliding_window_features.collaboration_experience.concrete_implementations.intra_shared_experience import build_features as intra_build_features
    from python_proj.data_preprocessing.sliding_window_features.collaboration_experience.concrete_implementations.eco_shared_experience import build_features as eco_build_features

    def build_features() -> Tuple[List[SharedExperienceFeature], List[SharedExperienceFeature]]:
